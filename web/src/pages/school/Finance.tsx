...
import { Badge, Button, Empty, ErrorBox, Field, FileLink, Form, Loading, Modal, Table, useAsync } from "../../ui/kit";
    ...
    <>
      ...
      <div className="grid two">
        <div className="card">
          ...
          {structures.loading ? <Loading /> : structures.error ? <ErrorBox error={structures.error} /> : <Table headers={["Name", "Amount"]} rows={(structures.data || []).map((s) => [s.name, money(s.tuitionAmount)])} />}
          {ops ? (
        ...
        {ops ? (
          ...
          <div className="card">
            ...
            {unpaid.loading ? <Loading /> : unpaid.error ? <ErrorBox error={unpaid.error} /> : (
      ...
      {ops ? (
        <div className="card" style={{ marginTop: 16 }}>
          ...
          {pending.loading ? <Loading /> : pending.error ? <ErrorBox error={pending.error} /> : !(pending.data || []).length ? <Empty title="Nothing to review" /> : (
            <Table
  ...
  return (
    <>
      <div className="page-title"><div><h1>Payroll</h1><p>Staff profiles and monthly generation</p></div>
        {ops ? <Button kind="brass" onClick={async () => { const rows = await salaryApi.generate(month); toast("ok", `${rows.length} salary rows`); void monthRows.reload(); }}>Generate this month</Button> : null}
      </div>
      {ops ? (
      ...
      {monthRows.loading ? <Loading /> : monthRows.error ? <ErrorBox error={monthRows.error} /> : (
        <Table
          ...
          rows={(monthRows.data || []).map((s) => [
            ...
            !ops || s.status === "PAID" ? pretty(s.status) : <Button key={`pay${s.id}`} onClick={async () => { await salaryApi.pay(s.id, today()); toast("ok", "Marked paid"); void monthRows.reload(); }}>Mark paid</Button>,
          ])}
        />
      )}
    </>
